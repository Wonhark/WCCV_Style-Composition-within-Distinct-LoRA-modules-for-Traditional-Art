...
class LoRAManager:
    ...
    def set_active_lora(self, adapter_name=None,  adapter_name2=None, weight=1.0):
        """활성 LoRA 설정"""
        try:
            if adapter_name is None:
                ...
                if adapter_name2 is not None:
                    self.pipe.set_adapters([adapter_name, adapter_name2], adapter_weights=[weight, weight])
                else:
...
class MultiFluxControlNetPipeline(nn.Module):
    ...
    def __call__(
        ...
        self.lora_manager.set_active_lora(adapter_name="style_1",  adapter_name2="style_2", weight=0.5)
        (
            prompt_embeds_1,
        ...
        with self.pipe.progress_bar(total=num_inference_steps) as progress_bar:
            for i, t in enumerate(timesteps):
                ...
                latents = latents + (sigma_next - sigma_t) * noise_pred_1
...
def main(args):
    ...
    prompt_list = [
        "a Korean palace with pine trees",
        "a woman in a hanbok dancing under cherry blossoms",
        "a woman in a hanbok standing under a pine tree",
        "a cat in front of the Korean palace",
        "a traditional Korean village in autumn",
        "a Korean folk painting of tigers and magpies",
    ]
    style_prompt_list = [
        "a Korean palace with pine trees, zzv style",
        "a woman in a hanbok dancing under cherry blossoms, zzv style",
        "a woman in a hanbok standing under a pine tree, zzv style",
        "a cat in front of the Korean palace, zzv style",
        "a traditional Korean village in autumn, zzv style",
        "a Korean folk painting of tigers and magpies, zzv style",
    ]
    ...
    for seed in seed_list:
        for prompt, style_prompt in zip(prompt_list, style_prompt_list):
            ...
            output_mix_1.save(f"{output_dir}/mixing_lora_fusion.png")
            # output_mix_2.save(f"{output_dir}/mixing2.png")
