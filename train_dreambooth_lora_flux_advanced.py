...
def log_validation(
    ...
    logger.info(
        f"Running validation... \n Generating {pipeline_args['prompt']} images with prompt:"
...
def parse_args(input_args=None):
    ...
    ### token_abstraction == placeholder_token
    parser.add_argument(
        "--token_abstraction",
        type=str,
        default="TOK",
    ...
    #### initializer_concept == superclass. it should be something like "man", ..., not "trp"
    parser.add_argument(
        "--initializer_concept",
        type=str,
        default=None,
    ...
    parser.add_argument(
        ...
        default=False,
        action="store_true",
        help="whether it is style personalization.",
    )
    parser.add_argument(
        "--entity_class",
        type=str,
        default=None,
        help="Not style training, entity personalization",
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--output_dir",
        type=str,
        default="flux-dreambooth-lora-advanced",
...
### Wonhark: added functions
def get_token_idx_in_prompt(tokenizer, prompts, placeholder_tokens, initializer_tokens):
    prompt_dict_list = []
    initializer_token_ids = []
    for token in initializer_tokens:
        initializer_token_ids.append(
            int(tokenize_prompt(tokenizer, token)['input_ids'][0][1]))
    placeholder_token_ids = tokenizer.convert_tokens_to_ids(placeholder_tokens)
    token_names = placeholder_tokens + initializer_tokens
    token_ids = placeholder_token_ids + initializer_token_ids

    for prompt in prompts:
        token_dict = {}
        input_token_ids = \
            tokenize_prompt(tokenizer, prompt)['input_ids'][0]
        count = 0
        for token, id in zip(token_names, token_ids):
            index_list = torch.isin(input_token_ids, torch.tensor(id))
            index_tensor = torch.nonzero(index_list, as_tuple=False)
            if len(index_tensor) == 0:
                continue
            elif len(index_tensor) == 2:
                token_dict[token + f"{count}"] = int(index_tensor[count])
                count = count + 1
            else:
                token_dict[token] = int(index_tensor)
        index_list = torch.isin(input_token_ids, tokenizer.eos_token_id)
        index_tensor = int(torch.nonzero(index_list, as_tuple=False)[0])
        token_dict["eos"] = index_tensor
        prompt_dict_list.append(token_dict)

    return prompt_dict_list


### if is_add_placeholder_modifier=True, then add placeholder_modifier to prompts.
### if is_placeholder_modifier_prior_images=True, then it is the case of is_same_class=True, and where modifier is added for each concepts. e.g. "glasses man", "long hair man"
def extract_prompts_from_image_list(list_of_directory_path, list_of_m_and_c=None, is_add_placeholder=None,
                                    is_placeholder_modifier_prior_images=None):
    import os
    import re
    prompts = []

    def make_nth_replacer(replace_word: str, n: int):
        def replace_nth(match):
            replace_nth.count += 1
            return replace_word if replace_nth.count == n else match.group(0)

        replace_nth.count = 0
        return replace_nth

    # Loop through files in the directory
    for filename_path in list_of_directory_path:
        filename = str(filename_path)
        if "--" in filename:
            # Split the filename into integer and prompt parts
            parts = filename.split("--", 1)
            if len(parts) == 2:
                # Extract the prompt (second part)
                prompt = parts[1]
                # Remove the file extension (if any)
                prompt = os.path.splitext(prompt)[0]
                # below repeated_num, subs~list is for the case of the same class
                repeated_num = 1
                substituted_c_token_list = []
                for m_and_c in list_of_m_and_c:
                    m_token = m_and_c[0]
                    c_token = m_and_c[1]
                    if is_placeholder_modifier_prior_images:
                        m_token = m_and_c[0] + ' ' + m_and_c[2]

                    if is_add_placeholder:
                        if c_token in substituted_c_token_list:
                            repeated_num += 1
                        replacer = make_nth_replacer(m_token + " " + c_token, repeated_num)
                        prompt = re.sub(rf'\b{c_token}\b', replacer, prompt)
                        substituted_c_token_list.append(c_token)

                prompts.append(prompt)
    # # Save the prompts to a .txt file
...
def get_random_train_prompt(concept_1, concept_2=None):
    start_list = ["", "a photo of ", "a rendering of ", "a cropped photo of ", "the photo of ", "a bright photo of ",
                  "a close-up photo of ", "a good photo of ", "a close-up photo of ", "a rendition of ",
                  "a dark photo of ", "a image of ", "a realistic image of "]
    adj_list = ["", "clean ", "dirty ", "cool ", "nice ", "small ", "weird ", "large "]
    a_list = ["", "a ", "the "]
    connect_list = ["and ", "besides ", "with ", "along with "]
    if concept_2 is not None:
        return random.choice(start_list) + random.choice(a_list) + random.choice(
            adj_list) + concept_1 + " " + random.choice(connect_list) + random.choice(a_list) + random.choice(
            adj_list) + concept_2
    else:
        return random.choice(start_list) + random.choice(a_list) + random.choice(adj_list) + concept_1
...
def main(args):
    ...
    if args.train_text_encoder_ti:
        ...
        for token_abs, token_replacement in token_abstraction_dict.items():
            ...
            if args.validation_prompt:
                for i, val_prompt in enumerate(args.validation_prompt):
                    args.validation_prompt[i] = val_prompt.replace(token_abs, "".join(token_replacement))
    ...
    if accelerator.is_main_process:
        ...
        accelerator.init_trackers(tracker_name, config=vars(args), init_kwargs={"wandb": {"name": args.proj_name}})
    ...
    if args.is_cross_concept_loss:
        cross_concept_losses = []
    else:
        cross_concept_losses = None
    for epoch in range(first_epoch, args.num_train_epochs):
        ...
        for step, batch in enumerate(train_dataloader):
            ...
            ##### Validation & Checkpointing saving process
            if accelerator.is_main_process:
                if args.validation_prompt is not None and (
                            global_step % args.validation_epochs == 0
                            # or global_step == 1
                    ):
                    ### save checkpoints
...
if __name__ == "__main__":
    ...
    ### 여기서는 initializer_token == token_abstraction
    # args.placeholder_token = ['<new1>']
    # args.initializer_token = 'ktn+pll+ucd'
    # args.initializer_token = 'zzv+ktn+pll+ucd'
    # args.token_abstraction = 'zzv'

    # if args.train_text_encoder_ti:
    token_abs = args.token_abstraction.split(",")[0]

    if args.is_style:   # style personalization
        if args.initializer_concept is not None:    # style with entity, such as folk_painting
            args.validation_prompt = [
                f'{token_abs} style {args.initializer_concept}',
                f'{token_abs} style {args.initializer_concept} in the mountain',
                f'{token_abs} style {args.initializer_concept} in traditional Korean style image',
            ]
        else:
            args.validation_prompt = [
                f'{token_abs} style image',
                f'dog in {token_abs} style',
                f'natural landscape in {token_abs} style',
            ]
    else:       # entity personalization
        if args.initializer_concept is not None:
            token_abs = token_abs + ' ' + args.initializer_concept
        args.validation_prompt = [
            f'{token_abs}',
            f'{token_abs} in New York',
        ]
